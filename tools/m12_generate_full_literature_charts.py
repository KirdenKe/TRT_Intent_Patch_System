...
def write_outputs(rows: list[dict[str, Any]], summary: dict[str, Any], refs: dict[str, Any], out_dir: Path) -> None:
    ...
    (out_dir / "vllm_prompt_path_audit.md").write_text(
        "\n".join(
            [
                ...
                "- trt-api then builds a structured chat-completions request using `VLLM_CHAT_COMPLETIONS_URL`, defaulting to `http://192.168.50.168:29987/v1/chat/completions`.",
    ...
    discussion = [
        ...
        "",
        "The active workflow uses two different vLLM-related paths. User-facing response formatting is sent directly to `http://192.168.50.168:29987/v1/chat/completions` from the n8n node `vLLM Format User Response`. Dialogue classification is routed through `trt-api` at `/chat/dialogue-decision`, which then builds a structured chat-completions request using `VLLM_CHAT_COMPLETIONS_URL` and a system prompt in `trt_core/api.py`.",
        "",
